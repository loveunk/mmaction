...
def main():
    ...
    with open(os.path.join(anno_path, "classids.json")) as f:
    ...
    f = open(os.path.join(anno_path, "kinetics400_train_list_videos.txt"), "w")
    for video in video_list:
        f.write(video.split('.')[0] + " " + str(video_label2id[video.split('/')[-2]]) + "\n")
    f.close()

    # generate val.csv
    video_list = glob.glob(val_path+"*/*.mp4")
    f = open(os.path.join(anno_path, "kinetics400_val_list_videos.txt"), "w")
    for video in video_list:
        f.write(video.split('.')[0] + " " + str(video_label2id[video.split('/')[-2]]) + "\n")
    f.close()
